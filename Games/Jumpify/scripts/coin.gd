...
func _on_body_entered(body):
    ...
    if body.has_method("is_player") or body.name == "Player" or body.is_in_group("player"):
        collected = true
        play_collection_effect()

func _on_visible_on_screen_notifier_3d_screen_exited() -> void:
    if not collected:  # Only free if not collected (to avoid interfering with collection effect)
        queue_free()
...
func play_collection_effect():
    ...
    if coin_sound and is_instance_valid(coin_sound):
        coin_sound.play()
    ...
    # Wait for tween to finish before destroying
