...
func plot_workspace_data(workspace_data: Array):
	if workspace_data.is_empty():
		return

	var x_labels = []
	var active_workspace = []
	var training_workspace = []
	var polygon_area = []

	for data in workspace_data:
		x_labels.append(data.date)
		active_workspace.append(data.axdir)   # you can adjust based on real field
		training_workspace.append(data.txdir) # you can adjust based on real field
		polygon_area.append(data.azdir)        # you can adjust based on real field

	var x_indexes = range(x_labels.size())

	# Create Function curves
	var f_active = Function.new(
		x_indexes, active_workspace, "Active Workspace",
		{
			color = Color("#36a2eb"),
	...
	var f_training = Function.new(
		x_indexes, training_workspace, "Training Workspace",
		{
			color = Color("#ff6384"),
	...
	var f_polygon = Function.new(
		x_indexes, polygon_area, "Polygon Area",
		{
			color = Color("#4bc0c0"),
	...
	cp.show_legend = true
	cp.title = "Patient Progress - Workspace Area"
	cp.x_label = "Session Date"
	cp.y_label = "Area / Workspace"
	cp.x_scale = 1
	cp.y_scale = 5
	cp.interactive = true

	plot([f_active, f_training, f_polygon], cp)
