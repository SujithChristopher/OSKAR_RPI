...
func _ready():
    ...
    var parse_files = preload("res://Results/Scripts/parse_files.gd").new()
    # parse_files._ready()
    ...
    cp.show_legend = true
    cp.title = "NOARK Assessment"
    cp.x_label = "Session"
    cp.y_label = "cm squared"
    cp.x_scale = 5
    cp.y_scale = 10
    ...
    f1 = Function.new(
        x, y, "Workspace", # This will create a function with x and y values taken by the Arrays 
    ...
    f2 = Function.new(x, y2, "Speed", { color = Color("#ff6384"), marker = Function.Marker.CROSS })
    f3 = Function.new(x, y3, "Smoothness", { color = Color.GREEN, marker = Function.Marker.TRIANGLE })
